...
from aws_cli_mcp.smithy.loader import _iter_json_files, load_models
...
def mock_fs(tmp_path):
    ...
    (ec2_dir / "ec2-2016.json").write_text(json.dumps({"shapes": {"ns#EC2": {"type": "service"}}}))
...
def test_iter_json_files_returns_empty_for_non_directory_path(tmp_path):
    path = tmp_path / "not-json.txt"
    path.write_text("x", encoding="utf-8")
    assert _iter_json_files(path) == []


def test_resolve_service_model_paths_skips_version_without_any_json(tmp_path):
    service_dir = tmp_path / "models" / "svc" / "2024-01-01"
    service_dir.mkdir(parents=True)
    # No JSON file in latest version directory
    resolved = resolve_service_model_paths(str(tmp_path), ["svc"])
    assert "svc" not in resolved
