...
def test_schema_generator_complex_shapes():
    ...
    # Define shapes
    # Map
    map_shape = MapShape("map", "map", {}, key=Member("str", {}), value=Member("int", {}))
    # Union
    ...
    str_shape = StringShape(
        "str", "string", {"smithy.api#pattern": "^[a-z]+$", "smithy.api#sensitive": {}}
    )

    # Integer with range
    int_shape = Shape("int", "integer", {"smithy.api#range": {"min": 0, "max": 10}})

    # Operation
    op_shape = OperationShape(
        "op", "operation", {}, input="struct", output=None, documentation=None, examples=None
    )

    # Input Struct
    struct_shape = StructureShape(
        "struct",
        "structure",
        {},
        {
...
def test_schema_generator_traits_length():
    model = MagicMock(spec=SmithyModel)
    str_shape = StringShape("str", "string", {"smithy.api#length": {"min": 5, "max": 10}})
...
def test_schema_generator_examples():
    ...
    struct_shape = StructureShape(
        ...
        {},
        {
            ...
            "O": Member(
                "int", {"required": False}
            ),  # Optional should be skipped in example depth > 0? No, checking logic
        },
    )
    ...
    op_shape = OperationShape(
        ...
        output=None,
        documentation=None,
        examples=[{"title": "Ex1", "input": {"S": "Explicit"}}],
    )
...
def test_schema_generator_missing_coverage():
    ...
    op_no_input = OperationShape(
        "op_no_input", "operation", {}, input=None, output=None, documentation=None, examples=None
    )
...
def test_schema_generator_depth_limit() -> None:
    """_shape_to_schema returns {"type": "object"} when depth >= _MAX_SCHEMA_DEPTH."""
    from aws_cli_mcp.smithy.schema_generator import _MAX_SCHEMA_DEPTH

    model = MagicMock(spec=SmithyModel)
    struct = StructureShape("deep", "structure", {}, {"a": Member("deep", {})})
    model.get_shape.return_value = struct

    gen = SchemaGenerator(model)
    result = gen._shape_to_schema("deep", {}, depth=_MAX_SCHEMA_DEPTH)
    assert result == {"type": "object"}


def test_schema_generator_unknown_primitive_and_example_fallback() -> None:
    model = MagicMock(spec=SmithyModel)
    custom_shape = Shape("custom", "customType", {})
    model.get_shape.side_effect = lambda sid: {"custom": custom_shape}.get(sid)

    gen = SchemaGenerator(model)
    schema = gen._shape_to_schema("custom", {})
    assert schema["type"] == "string"
    assert gen._example_for_shape("custom") == "string"
