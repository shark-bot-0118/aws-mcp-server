...
def test_catalog_indexing(mock_model):
    ...
    # Case-insensitive/snake-case operation lookup
    assert catalog.find_operation("s3", "list_buckets") is not None
...
def test_search(mock_model):
    ...
    # Exact match query
    results = list(catalog.search("ListBuckets"))
    assert len(results) >= 1
    assert results[0].ref.operation == "ListBuckets"

    # Service filter
    results = list(catalog.search("Bucket", service="s3"))
    assert len(results) >= 2  # List & Create

    # Documentation match
    results = list(catalog.search("Lists all"))
    assert len(results) == 1
    assert results[0].ref.operation == "ListBuckets"

    # No match
    results = list(catalog.search("NonExistentThing"))
    assert len(results) == 0

    # Empty query terms
    assert list(catalog.search("")) == []

    # Service filter mismatch path
    assert list(catalog.search("Bucket", service="ec2")) == []


def test_catalog_skips_non_operation_targets():
    model = MagicMock(spec=SmithyModel)
    service_shape = MagicMock(spec=ServiceShape)
    service_shape.shape_id = "ns#Service"
    service_shape.traits = {"aws.api#service": {"endpointPrefix": "svc"}}
    service_shape.operations = ["ns#NotOperation"]

    non_operation_shape = MagicMock()  # Not an OperationShape instance
    model.shapes = {
        "ns#Service": service_shape,
        "ns#NotOperation": non_operation_shape,
    }
    model.get_shape.side_effect = model.shapes.get

    catalog = SmithyCatalog(model)
    assert list(catalog.list_operations()) == []
